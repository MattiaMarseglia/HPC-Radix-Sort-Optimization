...
def extract_from_optimization_folders(root_opt_folders):
    ...
    for optimization_folder in optimization_folders:
        ...
        optimization_path = os.path.join(root_opt_folders, optimization_folder)
        # call function that extract measurements
        extract_from_schedule_folders(optimization_path)

# @brief call from main function to extract folders for different schedule
# @param root = the root were save measurements extracted organized for schedule


def extract_from_schedule_folders(root_schedule_folders):
    # obtain all directory that respect folders_condition for the measurement
    schedule_folders = [f for f in os.listdir(root_schedule_folders) if (
        folders_condition(root_schedule_folders, f, "SCHEDULE"))]
    for schedule_folder in schedule_folders:
        # obtain path were find schedule
        schedule_path = os.path.join(root_schedule_folders, schedule_folder)
        print(f"Found Folder : {schedule_folder}")
        if "AUTO" in schedule_folder:
            create_table_from_measure(_extract(schedule_path), schedule_path)
        else:
            extract_from_chunk_folders(schedule_path)

# @brief call from main function to extract folders for different chuck size
# @param root = the root were save measurements extracted organized for chuck size


def extract_from_chunk_folders(root_chunk_folders):

    chunk_folders = [f for f in os.listdir(root_chunk_folders) if
                     (folders_condition(root_chunk_folders, f, "CHUNK_SIZE-[0-9]"))]

    for chunk_folder in chunk_folders:
        print(f"Found Sub-Folder: {chunk_folder}")
        complete_path = os.path.join(root_chunk_folders, chunk_folder)
        # call function that extract measurements
...
def create_table_from_measure(subdir_where_measure, complete_path):
    # save header
    header = {
        'values': ['Version', 'Threads', 'Init', 'Real_Func', 'User', 'Sys', 'Elapsed', 'Speedup',
                   'Efficiency']}
...
def file_condition(file):
    if os.path.isfile(file) and file.endswith(".csv") and re.match("N_THREAD-[0-9]", file):
        return True
    return False
